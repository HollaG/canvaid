      ...
      <Stack spacing={6}>
        ...
        <Grid gridTemplateColumns={{ base: "1fr", md: "200px 1fr" }}>
          <GridItem p={5}>
            <Stack>
              <Button
                variant={selectedAttemptIndex === -1 ? "solid" : "outline"}
                colorScheme="green"
              ...
              {quiz.submissions.map((submission, i) => (
                <Button
                  variant={selectedAttemptIndex === i ? "solid" : "ghost"}
                  colorScheme="green"
                  key={i}
                  textAlign="left"
          ...
          <GridItem p={5}>
            {selectedAttemptIndex === -1 ? (
              <CombinedQuestionList quiz={quiz} setQuiz={setQuiz} />
            ) : (
              <Stack>
                <Heading fontSize="xl">
                  Attempt {quiz.submissions[selectedAttemptIndex].attempt}
                </Heading>
                <Divider />
                <Stack spacing="10">
                  {getQuestionsForAttempt(selectedAttemptIndex).map(
                    (question, i) => (
                      <Stack
                        key={i}
                        alignItems="stretch"
      ...
      <form onSubmit={(event) => handleSubmitAnnotation(event, question.id)}>
        <Flex>
          <Input
            ...
            variant="flushed"
          />
          <Button type="submit" colorScheme={"green"} size="sm" variant="ghost">
            Submit
          </Button>
        </Flex>
      </form>
...
const QuestionResultTag = ({
  ...
  if (questionResponse.your_score === questionResponse.total_score) {
    return (
      <Tag colorScheme="green">
        Correct! ({questionResponse.your_score} / {questionResponse.total_score}
        )
      </Tag>
    );
  ...
  if (questionResponse.your_score !== questionResponse.total_score) {
    return (
      <Tag colorScheme="yellow">
        Partial! ({questionResponse.your_score} / {questionResponse.total_score}
        ){" "}
      </Tag>
    );
...
const AnswerList = ({
  ...
  switch (questionType) {
    ...
    case "true_false_question":
      return (
        <RadioGroup
          value={selectedOptions.selected_answer_ids?.[0].toString() ?? "0"}
        >
          <Stack>
            {answers.map((answer, i) => (
              <Flex alignItems={"center"} key={i}>
                ...
                <Radio key={i} value={answer.id.toString()} isReadOnly>
                  {" "}
          ...
          <Stack spacing={4}>
            {answers.map((answer, i) => (
              <Flex alignItems={"center"} key={i}>
                ...
                <Checkbox key={i} value={answer.id.toString()} isReadOnly>
                  {" "}
                  {answer.text ?? answer.html}{" "}
                </Checkbox>
              </Flex>
            ...
            <Flex alignItems={"center"}>
              ...
              <Box>
                <Stack spacing={1}>
                  <Text fontWeight="semibold" textDecoration={"underline"}>
                    Correct answer
                    {selectedOptions?.correct_answer_text?.length > 1
                      ? "s"
                      : ""}
                  </Text>{" "}
                  {selectedOptions.correct_answer_text?.map((ans, i) => (
                    <Text key={i}>{ans}</Text>
                  ))}
                </Stack>
              </Box>
...
const CombinedQuestionList = ({
  ...
  return (
    <Stack spacing="10">
      <Heading fontSize="xl"> Showing best results for each question </Heading>
        ...
        <Box key={i}>
          ...
          <Tabs>
            <TabList>
              <Tab>
                {" "}
                Best attempt (#{question.best_attempt_number} -{" "}
              ...
              {question.attempts
                .map((attempt, i) => ({
                  v: (
                    <Tab key={i}>
                      {" "}
                      #{i + 1} ({attempt.your_score} / {attempt.total_score}){" "}
                    </Tab>
                  ),
                  i,
                }))
                .filter((d) => d.i !== question.best_attempt_number - 1)
                .map((d) => d.v)}
            </TabList>
            <TabPanels>
              <TabPanel>
                <AnswerList
                  ...
                  show_correct_answers={quiz.quizInfo.show_correct_answers}
                />
              </TabPanel>
              {question.attempts
                .map((attempt, i) => ({
          ...
          <QuestionExtras question={question} quiz={quiz} setQuiz={setQuiz} />
        </Box>
      ))}
    </Stack>
  );
